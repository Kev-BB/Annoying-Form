...
Aşağıdaki hata mesajlarından birini alabilirsiniz:
    ...
    Uyarı: Select elementleri kontrollü ya da kontrolsüz olmalıdır (value prop ya da defaultValue prop belirtin, ancak ikisini birden belirtmeyin). Kontrollü veya kontrolsüz bir seçim elementi kullanmak arasında karar verin ve bu prop'lardan birini kaldırın.

        veya

    Uyarı: App, hem checked hem de defaultChecked prop'larına sahip radio türünde bir input içeriyor. Input elementleri ya controlled ya da uncontrolled olmalıdır (checked prop'unu ya da defaultChecked prop'unu belirtin, ancak ikisini birden belirtmeyin). Kontrollü veya kontrolsüz bir input elemanı kullanmak arasında karar verin ve bu proplardan birini kaldırın.
